...
TypeScriptの`interface`はオープンエンドなので、`lib.d.ts`で宣言されたインターフェースにメンバを追加するだけで、TypeScriptはその追加を認識します。これらのインターフェースを`lib.d.ts`に関連付けるには、これらの変更を [グローバルモジュール](../project/modules.md) で行う必要があることに注意してください。このために、 [`global.d.ts`](../project/globals.md) という特別なファイルを作成することをお勧めします。
